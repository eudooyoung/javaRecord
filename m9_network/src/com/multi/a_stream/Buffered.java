...
public class Buffered {
     ...
     * 필터스트림(보조스트림)은 외부 데이터에 직접 연결하는 것이 아니라 기본 스트림에 추가로 사용할 수 있는 스트림이다.
...
//2. 보조스트림생성(위에서 만들어진 기반스트림의 성능 좀 더 향상시키고자 함) ,매개변수로 기반스트림을 사용
    ...
    public void fileSave() {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("buffer.txt"))) {  //보조 스트림 안에 본 스트림
    ...
    public void fileOpen() {
        try (BufferedReader bw = new BufferedReader(new FileReader("buffer.txt"))) {  //보조 스트림 안에 본 스트림

//            System.out.println(bw.readLine());
//            System.out.println(bw.readLine());
//            System.out.println(bw.readLine());
//            System.out.println(bw.readLine()); // 읽어올 문자열이 없으면 null 출력
